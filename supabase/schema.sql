-- =====================================================
-- Lead Management CRM - Complete Schema
-- =====================================================
...
CREATE POLICY "leads_delete_restricted" ON leads
  ...
  USING (
    ...
    AND get_current_user_role() IN ('super_admin', 'admin', 'sales_manager')
  );
...
CREATE POLICY "activities_select_org" ON activities
  FOR SELECT
  USING (organization_id = get_current_user_org_id());

CREATE POLICY "activities_insert_org" ON activities
  FOR INSERT
  WITH CHECK (organization_id = get_current_user_org_id());

CREATE POLICY "activities_update_org" ON activities
  FOR UPDATE
  USING (organization_id = get_current_user_org_id());

CREATE POLICY "activities_delete_org" ON activities
  FOR DELETE
  USING (organization_id = get_current_user_org_id());
...
CREATE POLICY "source_stats_select_org" ON lead_source_stats
  FOR SELECT
  USING (organization_id = get_current_user_org_id());

CREATE POLICY "source_stats_insert_org" ON lead_source_stats
  FOR INSERT
  WITH CHECK (organization_id = get_current_user_org_id());

CREATE POLICY "source_stats_update_org" ON lead_source_stats
  FOR UPDATE
  USING (organization_id = get_current_user_org_id());

CREATE POLICY "source_stats_delete_org" ON lead_source_stats
  FOR DELETE
  USING (organization_id = get_current_user_org_id());
...
-- Auto-generate lead numbers (LEAD-YYYYMMDD-NNNN)
...
-- Log lead activities automatically
...
BEGIN
  IF TG_OP = 'INSERT' THEN
    ...
    VALUES (
      ...
      'lead_created',
      'Lead created',
      jsonb_build_object('lead_number', NEW.lead_number, 'source', NEW.lead_source),
      NEW.owner_id,
      NEW.organization_id
    );
...
-- Update lead_source_stats when leads are created
...
CREATE TRIGGER on_lead_source_stats
  AFTER INSERT ON leads
  FOR EACH ROW EXECUTE FUNCTION update_lead_source_stats();
